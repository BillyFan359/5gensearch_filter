...
for line in seedLst:
    ...
    parsedKeys[0] = parsedKeys[0][1:]
    parsedKeys[len(parsedKeys)-1]=parsedKeys[len(parsedKeys)-1].split("\n")[0]
    parsedKeys[len(parsedKeys)-1]=parsedKeys[len(parsedKeys)-1][:len(parsedKeys[len(parsedKeys)-1])-1]
    for x in range(0,len(parsedKeys)):
        if parsedKeys[x] == "SELECT":
            parsedKeys[x] = "Se"
        if parsedKeys[x] == "START":
            parsedKeys[x] = "St"
    keystr = ""
    for x in parsedKeys:
        keystr+=x
